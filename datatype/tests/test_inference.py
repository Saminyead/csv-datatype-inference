...
@pytest.fixture
def df_to_infer_test(filename:str=SAMPLE_DATA_PATH) -> DataFrameToInfer:
    """Test Dataframe fixture"""
    df = pd.read_csv(filename)
    return DataFrameToInfer(df)


def test_infer_numeric_80_pct_col_is_numeric(df_to_infer_test:DataFrameToInfer) -> None:
...
def test_infer_datetime_varied_dt_formats(
        df_to_infer_test:DataFrameToInfer
) -> None:
